		...
		<div class="modal-wrapper">
			<div class="modal-container">
				...
				<div class="modal-header">
					<div name="header">
						<h1 class="title">{{ modalContent.title }}</h1>
					</div>
				</div>
				<div class="modal-body">
					<div name="body">
						<section class="email">
							...
							<form @submit.prevent="sendEmail" v-if="!isEmailSent">
								<input type="text" placeholder="이메일을 입력하세요." v-model="email" />
								<button type="submit">{{ modalContent.title }}</button>
							</form>
							<div class="email-success" v-else>
