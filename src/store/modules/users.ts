...
import { getUserProfile, getPublicUserProfile, updateUser } from '@/services/api/users';
...
export const actions: ActionTree<UsersState, RootState> = {
  ...
  async fetchPublicProfile({ commit }, userLoginId) {
    const { data } = await getPublicUserProfile(userLoginId);
    commit('SET_PUBLIC_PROFILE', data);
    return data;
  },
  // 사용자 프로필 변경
  async updateProfileImage({ commit }, profileImage) {
